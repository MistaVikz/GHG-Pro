...
By default, this script will generate 100 test projects. You can specify the number of test projects to generate by passing an optional command line argument, like this:
python scripts/generate_project_risk_data.py 10

Replace `10` with the desired number of test projects.
...
6. `run_simulation(df_project, num_samples=10000):` This function runs a simulation to calculate the projected delivery volume and its standard deviation for each year in a project. It takes in a DataFrame with expected values and standard deviations for each risk bucket, as well as the number of random samples to generate (default is 10000). The function generates random samples for each risk bucket using a normal distribution with the expected value and standard deviation for that bucket. It then calculates the projected delivery volume for each set of input samples by summing the samples across all risk buckets. The function calculates the standard deviation of the projected delivery volume for each year, as well as the overall project delivery (defined as the offered volume minus twice the standard deviation) and the expected value percentage (defined as the overall project delivery divided by the offered volume). The results are stored in new columns in the input DataFrame, which is returned by the function.
